...
#   timestamp: 2021-02-17T04:09:08+00:00
...
# from .genotyping.views import get_variants
...
@app.get('/brapi/v2/serverinfo', response_model=ServerinfoGetResponse, response_model_exclude_none=True)
def get_serverinfo(
...
from .genotyping.models import *
@app.get('/brapi/v2/variantsets', response_model=VariantsetsGetResponse, response_model_exclude_none=True)
def get_variantsets(
    ...
    """
    from .genotyping.models import VariantsetsGetResponse, DataFile, DataFormat, AvailableFormat, FileFormat
    from .models import Metadata
    result1 = VariantSet(
        # additionalInfo= {"original_datset":{'name':"HDRA6.4", "authors":"McCouch et al."}},
        # analysis= ["Alchemy", "Plink freq"],
        availableFormats= [
            AvailableFormat(dataFormat=DataFormat.tabular, fileFormat=FileFormat.application_excel, fileURL='https://localhost:9000/dataset/HDRA_Data.xls'),
            AvailableFormat(dataFormat=DataFormat.tabular, fileFormat=FileFormat.application_excel, fileURL='https://localhost:9000/dataset/HDRA_Data.xlsx'),
            AvailableFormat(dataFormat=DataFormat.VCF, fileFormat=FileFormat.application_excel, fileURL='https://localhost:9000/dataset/HDRA_Data.vcf'),           
            ])
    result2 = VariantSet(
        # additionalInfo= {"original_datset":{'name':"HDRA6.4", "authors":"McCouch et al."}},
        # analysis= ["Alchemy", "Plink freq"],
        availableFormats= [
            AvailableFormat(dataFormat=DataFormat.tabular, fileFormat=FileFormat.application_excel, fileURL='https://localhost:9000/dataset/HDRA_Data2.xlsx'),
            AvailableFormat(dataFormat=DataFormat.VCF, fileFormat=FileFormat.application_excel, fileURL='https://localhost:9000/dataset/HDRA_Data2.vcf'),
            AvailableFormat(dataFormat=DataFormat.VCF, fileFormat=FileFormat.application_zip, fileURL='https://localhost:9000/dataset/HDRA_Data2.vcf.gz'),
            ])


    metadata = Metadata(datafiles=[], status=[])
    # response = VariantsetsExtractPostResponse(metadata=metadata, result=result)

    result = [result1, result2]
    print(result)

    response = VariantsetsGetResponse(metadata=metadata, result={'data':result})
    return response
...
@app.post('/brapi/v2/variantsets/extract', response_model=VariantsetsExtractPostResponse, response_model_exclude_none=True)
def post_variantsets_extract(
    ...
    result = VariantSet(
        ...
        availableFormats= [
                {
                    'dataFormat': DataFormat.tabular,
                    'fileFormat': 'application/excel',
                    'fileURL': 'https://localhost:9000/dataset/HDRA_Data.xlsx',
                },
                {
                    'dataFormat': DataFormat.VCF,
                    'fileFormat': 'text/csv',
                    'fileURL': 'https://localhost:9000/dataset/HDRA_Data.vcf',
                },
            ])
    print(result)
    metadata = Metadata(datafiles=[], status=[])
